...
//🚀🚀🚀 ⬇️ 📝 Explanation ⬇️ 📝 🚀🚀🚀: 
// Because the nestedFunction() method is defined within the Closure of myFunction() (the outer function), it is considered a 'privileged method',
// thereby granting it access to data within the outer function's Closure. Thus, nestedFunction's Closure includes access to the declaration of 'internal'.
...
const zooAnimals = [
  ...
  */
  ...
  function animalNames(arr){
    const nameArray = [];
    arr.forEach(val => nameArray.push((`name: ${val["animal_name"]}, scientific: ${val["scientific_name"]}`)));
    return nameArray;
...
console.log(animalNames(zooAnimals));  
  ...
  */
  ...
  function lowerCaseNames(arr){
   return arr.map(val => val.animal_name.toLowerCase()); 
  }
  ...
  */
  ...
  function lowPopulationAnimals(arr){
    return arr.filter(val => val.population < 5);
  }
  ...
  */
  ...
  function USApop(arr){
    return arr.map(val => val.population).reduce((a,b) => a + b); 
  }
